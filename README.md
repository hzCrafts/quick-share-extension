...
- 🎨 **精美预设主题**：内置 3 种卡片主题（Raycast 曜石、冰川玻璃、暖纸墨色），默认采用 Raycast Wrapped 风格的深黑面板与明亮橙色辉光。
- ⚙️ **极简编辑器**：浅色毛玻璃弹窗，宽度最多 720px、高度最多 810px，无标题栏与侧栏；主题色球、复制／保存图标集中在单行底栏，长图支持有边界的滚动预览。
...
- 🌍 **通用网页摘录**：X、知乎、ChatGPT、Gemini 保留全文分享；其他普通 HTTP(S) 网页（含微信公众号文章）支持划线分享，自动读取文章标题、作者／站点与可识别的发布时间。
- 🌐 **多维度触发**：
  - X、知乎、ChatGPT、Gemini 内容操作栏自动注入的 QuickShare 按钮
...
│   ├── universal.adapter.ts # 其他网页：仅划线分享与来源信息
│   └── index.ts            # 专用优先，通用兜底
├── assets/                 # 样式与静态资源
...
└── utils/                  # 导出与通用工具
    ...
    ├── selection.ts        # 共享选区边界校验与 Range 切分
    ├── web-excerpt.ts      # 通用网页 HTML 与资源地址清洗
...
在 Chrome 中打开文章，选中文字后点击悬浮分享按钮，或使用右键菜单。微信公众号文章需在 Chrome 中打开 `mp.weixin.qq.com` 链接；此扩展不运行于微信内置浏览器。

- 保留选中文字、段落、换行、列表与图片的先后顺序，沿用三种主题及最多两行的渐隐上下文。
- 优先读取文章元数据；没有明确作者时将文章标题直接放在卡片顶部，不用站点名冒充作者；没有发布时间时省略日期。
- 公众号优先显示真实账号头像，缺失或加载失败时显示与编辑器一致的主题圆球。
- 不在其他网页注入全文分享按钮，不分享输入框、可编辑内容和导航栏里的选区。
- Chrome 内部页面、扩展商店等受保护页面无法注入；PDF 阅读器、画布文字、Shadow DOM 和 iframe 内的选区暂不覆盖。
- 更新扩展后，需要刷新已打开的网页再测试。
